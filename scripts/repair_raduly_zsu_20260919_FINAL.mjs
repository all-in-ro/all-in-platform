...
try {
  ...
  const paymentRes = await client.query(
    `SELECT p.id, p.customer_id, p.location_id, p.amount, p.method, p.paid_at,
            COALESCE(sum(a.amount),0)::numeric AS allocated_total,
            count(a.id)::int AS allocation_count
     FROM aif_shop_customer_payments p
     JOIN aif_shop_customer_payment_allocations a ON a.customer_payment_id=p.id
  ...
  if (paymentRes.rowCount !== 1) {
    throw new Error(`Nem egyetlen közös kliensbefizetést találtam a két eladáshoz, hanem ${paymentRes.rowCount}-t. Leálltam.`);
  }
