...
export default function AllInMagazinTargu({
  ...
  const refreshOpeningInventoryAccess = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/aif/opening-inventory/active?location=${encodeURIComponent("magazin_targu_secuiesc")}`,
        {
          credentials: "include",
          cache: "no-store",
  ...
  function openOpeningInventory() {
    ...
    try {
      window.sessionStorage.setItem("allin:opening-inventory:location", "magazin_targu_secuiesc");
    } catch {}
