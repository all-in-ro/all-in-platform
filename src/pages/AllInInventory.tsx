  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>
    ...
    * { box-sizing: border-box; }
    ...
    <header>
      ...
      <div class="meta">ALLINFASHION</div>
    </header>
    <div class="summary">
...
export default function AllInInventory({ actor = "ADMIN" }: Props) {
  ...
  const loadSessions = useCallback(async (locationValue: string, silent = false) => {
    ...
    try {
      ...
      if (!active) {
        ...
        setInventoryMode(hasApplied ? "standard" : "recovery");
      }
      if (active) {
        ...
        <header className="rounded-[22px] border border-white/16 bg-[#303a4c] px-4 py-3 shadow-[0_16px_42px_rgba(15,23,42,0.24)]">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[10px] uppercase tracking-[0.15em] text-white/45">ALLINFASHION</p>
              <h1 className="mt-1 text-2xl font-normal">Leltár</h1>
              <p className="mt-1 text-xs text-white/52">Üzletenkénti leltár, számolás és készletbevezetés • {actor}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => { window.location.hash = "home"; }} className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/18 bg-[#354153] px-3 text-xs text-white hover:bg-[#405066]"><ArrowLeft size={15} /> Kezdőlap</button>
