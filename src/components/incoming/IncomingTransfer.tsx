...
export default function IncomingTransfer(props: {
  ...
  async function commitSelected() {
    ...
    try {
      ...
      setMsg("Véglegesítve (könyvelve).");
      await refreshHistory();
    } catch (e: any) {
      setErr(e?.message || "Nem sikerült véglegesíteni.");
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              className="inline-flex items-center gap-2 rounded-lg border border-slate-900 bg-slate-900 px-3 py-2 text-sm text-white hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
              onClick={commitSelected}
              disabled={!selectedId || busy}
              type="button"
              title={selectedId ? "Mozgatás véglegesítése (készlet frissül)" : "Előbb mentsd a mozgatást (Mentés), majd véglegesíts"}
            >
                ...
                <tr key={h.id} className="border-t border-slate-200 text-slate-900">
                  ...
                  <td className="px-3 py-2 font-normal">
                    {h.status === "committed" ? (
                      ...
                      <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">tervezet</span>
                    )}
                  </td>
                </tr>
              ))}
