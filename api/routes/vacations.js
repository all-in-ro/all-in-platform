...
export default function createVacationsRouter({ pool, requireAuthed, requireAdminOrSecret }) {
  ...
  function sessionEmployeeName(req) {
    ...
    const candidates = [
      ...
      req.get?.("x-allin-employee"),
      req.query?.employee,
      req.body?.employeeName,
      req.body?.employee_name,
    ];
    for (const candidate of candidates) {
      const value = norm(candidate).replace(/\s+/g, " ");
      if (value && !["admin", "administrator", "system"].includes(value.toLowerCase())) return value;
    }
    return "";
  }

  async function requireVacationEmployee(req, res, next) {
    try {
      const candidate = norm(req.vacationEmployeeName) || sessionEmployeeName(req);
      if (!candidate) {
        return res.status(401).json({ error: "A dolgozó nem azonosítható." });
      }
      ...
      const employeeName = norm(employeeResult.rows?.[0]?.name);
      if (!employeeName) {
        return res.status(403).json({ error: "A bejelentkezett dolgozó nem található a felhasználók között." });
      }

      req.vacationEmployeeName = employeeName;
      next();
    } catch (error) {
      console.error("vacation employee identification failed", error);
      res.status(500).json({ error: "A dolgozó azonosítása nem sikerült." });
    }
  }
      ...
      const [requestsResult, eventsResult, summaryResult] = await Promise.all([
        pool.query(
          `SELECT id, employee_name AS "employeeName", shop_id AS "shopId", kind,
                  ...
                  decision_note AS "decisionNote", employee_seen_at AS "employeeSeenAt"
           FROM allin_time_off_requests
           WHERE lower(regexp_replace(btrim(employee_name), '\\s+', ' ', 'g')) =
    ...
    try {
      ...
      const result = await pool.query(
        `UPDATE allin_time_off_requests
         ...
         WHERE id=$1::uuid
           AND lower(regexp_replace(btrim(employee_name), '\\s+', ' ', 'g')) =
