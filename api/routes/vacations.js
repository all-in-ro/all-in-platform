...
export default function createVacationsRouter({ pool, requireAuthed, requireAdminOrSecret }) {
  ...
  async function requireVacationEmployee(req, res, next) {
    try {
      const requestedName = norm(
        req.vacationEmployeeName ||
        sessionEmployeeName(req) ||
      ...
      if (!employee.rowCount) {
        return res.status(403).json({ error: "A dolgozó nem található a felhasználók között." });
      }

      req.vacationEmployeeName = employee.rows[0].name;
      next();
    } catch (error) {
      console.error("vacation employee resolution failed", error);
